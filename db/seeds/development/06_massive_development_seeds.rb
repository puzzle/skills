...
def seed_companies(count)
  puts "-> Seeding #{count} extra companies..."  seeder = CompanySeeder.new
...
def seed_auth_users(count)
  puts "-> Seeding #{count} extra auth users..."  seeder = AuthUserSeeder.new
...
def seed_certificates(factor)
  ...
  total = factor * 20
  puts "-> Seeding #{total} extra certificates..."  seeder = CertificateSeeder.new
  factor.times { seeder.seed_certificates }
end

def seed_skills(count)
  puts "-> Seeding #{count} extra skills..."  suffixes = %w[Basics Advanced Expert Architecture Patterns Management Security Automation Framework Engineering Operations Design Strategy Analysis]
  ...
  count.times do
    ...
    def seed_people(count)
      puts "-> Seeding #{count} extra people (this will generate deep associations!)..."  seeder = PersonSeeder.new
