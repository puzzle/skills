...
describe Export::BaseCvExport do
  fixtures :people

  before { allow_any_instance_of(Export::BaseCvExport).to receive(:location).and_return(branch_adresses(:bern)) }
  ...
  subject { described_class.new(person, params) }
    ...
    it 'can export without an image' do
      ...
      Export::BaseCvExport.new(person, { 'anon' => 'false' })
    end
    ...
    it 'translates nationalities' do
      nationalities = Export::BaseCvExport.new(people(:bob), { 'anon' => 'false' }).send(:nationalities)
      expect(nationalities).to eq('Schweiz, Schweden')

      nationalities = Export::BaseCvExport.new(people(:alice), { 'anon' => 'false' }).send(:nationalities)
      expect(nationalities).to eq('Australien')
    end

    it 'get one skill hash when filter on level 3' do
      skills = Export::BaseCvExport.new(people(:bob), { 'skillsByLevel' => 'true' }).send(:skills_by_level_value, 3)
    ...
    it 'get empty skill hash when filter on level 5' do
      skills = Export::BaseCvExport.new(people(:bob), { 'skillsByLevel' => 'true' }).send(:skills_by_level_value, 5)

      expect(skills).to eq([])
    end

    it 'get PeopleSkill by level 2' do
      skills = Export::BaseCvExport.new(people(:bob), { 'skillsByLevel' => 'true' }).send(:skills_by_level, 2)
      people_skill = PeopleSkill.find 996949822
      expect(skills).to eq([people_skill])
    end

    it 'get no PeopleSkill by level 5' do
      skills = Export::BaseCvExport.new(people(:bob), { 'skillsByLevel' => 'true' }).send(:skills_by_level, 5)
      expect(skills).to eq([])
    end

    it 'formats competence notes' do
      notes = Export::BaseCvExport.new(people(:bob), { 'anon' => 'false' }).send(:competence_notes_list)[:competence]
  ...
  describe 'format language with level correctly' do
    let(:bob) { people(:bob) }
    let(:cv) { Export::BaseCvExport.new(bob, { 'anon' => 'false' }) }
