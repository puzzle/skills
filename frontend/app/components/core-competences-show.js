...
export default Component.extend({
  ...
  init() {
    ...
    this.parentCategories = this.get('store').query('category', { scope: 'parents' });
  ...
  refreshCoreCompetencesObj() {
    ...
    this.get('parentCategories').then(parentCategories => {
      parentCategories.toArray().forEach(category => {
        let skills = category.get('childrenSkills').map(skill => {
          if (this.get('coreCompetenceSkills').map(s => s.get('id')).includes(skill.get('id'))) return skill;
  ...
  coreCompetenceSkills: computed('person.peopleSkills', function() {
    return this.get('person.peopleSkills').map(ps => {
