...
module("Acceptance | skill search", function(hooks) {
  ...
  test("search peopleSearchSkills of Rails", async function(assert) {
    assert.expect(9);
    ...
    const names = page.indexPage.peopleSearchSkills.peopleNames
      .toArray()
      .map(name => name.text);
    ...
    const names2 = page.indexPage.peopleSearchSkills.peopleNames
      .toArray()
    ...
    const names3 = page.indexPage.peopleSearchSkills.peopleNames
      .toArray()
      .map(names3 => names3.text);
    assert.ok(names3.includes("Bob Anderson"));
  });

  test("search peopleSearchSkills of JUnit", async function(assert) {
    assert.expect(11);
    ...
    const names4 = page.indexPage.peopleSearchSkills.peopleNames
      .toArray()
  ...
  test("search multiple skills", async function(assert) {
    ...
    assert.equal(
      currentURL(),
      "/skill_search?interest=1%2C1&level=1%2C1&skill_id=" +
        rails.get("id") +
        "%2C" +
        cunit.get("id")
    );
